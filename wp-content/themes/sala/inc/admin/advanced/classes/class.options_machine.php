		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'group':
					...
					if (array_key_exists('subject_' . $value['id'], $smof_data) || array_key_exists('content_' . $value['id'], $smof_data)) {
						$t_subject = stripslashes($smof_data['subject_' . $value['id']]);
						$t_content = stripslashes($smof_data['content_' . $value['id']]);
					}
					$mini ='';
					if(!isset($value['mod'])) $value['mod'] = '';
					if($value['mod'] == 'mini') { $mini = 'mini';}

					$output .= '<div class="field-group ' . $mini . '">';
					$output .= '<div class="field-input"><h4 class="heading">' . esc_html__( 'SUBJECT', 'sala' ) . '</h4>';
					$output .= '<input class="of-input" name="subject_' . $value['id'] . '" id="subject_'. $value['id'] .'" type="text" value="'. $t_subject .'" />';
					$output .= '</div>';
					$output .= '<div class="field-input"><h4 class="heading">' . esc_html__( 'CONTENT', 'sala' ) . '</h4>';
					$output .= '<textarea class="of-input" name="content_' . $value['id'] . '" id="content_'. $value['id'] .'" rows="8">'.$t_content.'</textarea>';
					$output .= '</div></div>';
				break;
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'sala').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
