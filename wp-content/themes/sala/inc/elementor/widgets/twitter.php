...
class Widget_Twitter extends Base {
	...
	public function get_title() {
		return esc_html__( 'Twitter', 'sala' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control( 'consumer_key', [
			'label' => esc_html__( 'Consumer Key', 'sala' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'consumer_secret', [
			'label' => esc_html__( 'Consumer Secret', 'sala' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token', [
			'label' => esc_html__( 'Access Token', 'sala' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token_secret', [
			'label' => esc_html__( 'Access Token Secret', 'sala' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'username', [
			'label' => esc_html__( 'Twitter Username', 'sala' ),
			'type'  => Controls_Manager::TEXT,
		] );
		...
		$this->add_control( 'show_date', [
				...
				'label_on'     => esc_html__( 'Yes', 'sala' ),
				'label_off'    => esc_html__( 'No', 'sala' ),
				'return_value' => '1',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'sala-twitter style-list' );
		?>
		...
		if ( $username !== '' && $consumer_key !== '' && $consumer_secret !== '' && $access_token !== '' && $access_token_secret !== '' && $number_items !== '' ) {
			...
			if ( false === $twitter_data || isset( $json['errors'] ) ) {
				...
				if ( ! $token ) {
					...
					$to_send     = sala_core_base_encode( $credentials );
