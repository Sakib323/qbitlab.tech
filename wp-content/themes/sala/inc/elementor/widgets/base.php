...
abstract class Base extends Widget_Base {
	...
	public function get_icon() {
		return 'sala-badge ' . $this->get_icon_part();
	}
	...
	protected function render_common_button() {
		...
		$this->add_render_attribute( 'button', 'class', 'elementor-button style-' . $settings['button_style'] );

		if ( ! empty( $settings['button_size'] ) ) {
			$this->add_render_attribute( 'button', 'class', 'elementor-button-' . $settings['button_size'] );
		}
	...
	protected function register_common_button_style_section() {
		...
		$this->add_control( 'button_margin', [
			...
			'size_units' => [ 'px', '%' ],
			'selectors'  => [
				'{{WRAPPER}} .sala-button-wrapper' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'button_padding', [
			...
			'size_units' => [ 'px', '%' ],
			'selectors'  => [
				'{{WRAPPER}} .sala-button-wrapper .elementor-button' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'badge_rounded', [
			...
			'size_units' => [ 'px', '%' ],
			'selectors'  => [
				'{{WRAPPER}} .sala-pricing .elementor-button, {{WRAPPER}} .sala-pricing .elementor-button:before' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_group_control( Group_Control_Text_Gradient::get_type(), [
			'name'     => 'button_text',
			'selector' => '{{WRAPPER}} .elementor-button .button-text',
		] );

		/**
		 * Background Color
		 */
		$this->add_control( 'button_background_color_normal_heading', [
		...
		$this->add_group_control( Group_Control_Text_Gradient::get_type(), [
			'name'      => 'button_icon',
			'selector'  => '{{WRAPPER}} .elementor-button .button-icon',
			'condition' => $icon_condition,
		] );
		...
		$this->add_control( 'button_line_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .elementor-button.style-bottom-line .button-content-wrapper:before' => 'background: {{VALUE}};',
				'{{WRAPPER}} .elementor-button.style-left-line .button-content-wrapper:before'   => 'background: {{VALUE}};',
			],
			'condition' => $line_condition,
		] );
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			...
			'selector' => '{{WRAPPER}} .sala-box:hover div.elementor-button, {{WRAPPER}} a.elementor-button:hover',
		] );
		...
		$this->add_group_control( Group_Control_Text_Gradient::get_type(), [
			...
			'selector' => '{{WRAPPER}} .sala-box:hover div.elementor-button .button-text, {{WRAPPER}} a.elementor-button:hover .button-text',
		] );

		/**
		 * Background Color
		 */
		$this->add_control( 'hover_button_background_color_normal_heading', [
			'label'     => esc_html__( 'Background Color', 'sala' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'classes'   => 'control-heading-in-tabs',
		...
		$this->add_group_control( Group_Control_Text_Gradient::get_type(), [
			...
			'selector'  => '{{WRAPPER}} .sala-box:hover div.elementor-button .button-icon, {{WRAPPER}} a.elementor-button:hover .button-icon',
			'condition' => $icon_condition,
		] );
		...
		$this->add_control( 'hover_button_line_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .elementor-button.style-bottom-line .button-content-wrapper:after' => 'background: {{VALUE}};',
				'{{WRAPPER}} .elementor-button.style-left-line .button-content-wrapper:after'   => 'background: {{VALUE}};',
			],
			'condition' => $line_condition,
		] );
		...
		$this->add_control( 'button_icon_indent', [
			...
			'selectors' => [
				'{{WRAPPER}} .elementor-button.icon-left .button-icon'  => 'margin-right: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}} .elementor-button.icon-right .button-icon' => 'margin-left: {{SIZE}}{{UNIT}};',
			],
			'condition' => $icon_condition,
		] );
		...
		$this->add_responsive_control( 'button_icon_font_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .elementor-button .button-icon' => 'font-size: {{SIZE}}{{UNIT}};',
			],
			'condition' => $icon_condition,
		] );
